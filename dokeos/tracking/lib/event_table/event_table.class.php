...
class EventTable
{
     ...
     *                                                       default renderer.
     */
    function EventTable($data_provider, $table_name = null, $column_model = null, $cell_renderer = null)
    {
        $this->set_data_provider($data_provider);
        $this->set_name(isset($table_name) ? $table_name : self :: DEFAULT_NAME);
        $this->set_column_model(isset($column_model) ? $column_model : new DefaultEventTableColumnModel());
        $this->set_cell_renderer(isset($cell_renderer) ? $cell_renderer : new DefaultEventTableCellRenderer());
    ...
    function as_html()
    {
        $table = new SortableTable($this->get_name(), array($this, 'get_event_count'), array($this, 'get_events'), $this->get_column_model()->get_default_order_column() + ($this->has_form_actions() ? 1 : 0), $this->get_default_row_count(), $this->get_column_model()->get_default_order_direction());
     ...
     */
    function get_events($offset, $count, $order_column, $order_direction)
    {
        $events = $this->get_data_provider()->get_events($offset, $count, $this->get_column_model()->get_column($order_column - ($this->has_form_actions() ? 1 : 0))->get_event_property(), $order_direction);
        $table_data = array();
        $column_count = $this->get_column_model()->get_column_count();
        while ($event = $events->next_result())
        {
            ...
            for($i = 0; $i < $column_count; $i ++)
            {
                $row[] = $this->get_cell_renderer()->render_cell($this->get_column_model()->get_column($i), $event);
            }
            $table_data[] = $row;
        }
        return $table_data;
    ...
    {
        return $this->get_data_provider()->get_event_count();
    }
}
?>