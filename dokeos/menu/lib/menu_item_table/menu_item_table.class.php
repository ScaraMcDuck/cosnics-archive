...
require_once dirname(__FILE__).'/default_menu_item_table_column_model.class.php';
require_once dirname(__FILE__).'/default_menu_item_table_cell_renderer.class.php';
...
class MenuItemTable
{
	 ...
	 *                                                       default renderer.
	 */
	function MenuItemTable($data_provider, $table_name = null, $column_model = null, $cell_renderer = null)
	{
		$this->set_data_provider($data_provider);
		$this->set_name(isset($table_name) ? $table_name : self :: DEFAULT_NAME);
		$this->set_column_model(isset ($column_model) ? $column_model : new DefaultMenuItemTableColumnModel());
		$this->set_cell_renderer(isset ($cell_renderer) ? $cell_renderer : new DefaultMenuItemTableCellRenderer());
	...
	function as_html()
	{
		$table = new SortableTable($this->get_name(), array ($this, 'get_menu_items_count'), array ($this, 'get_menu_items'), $this->get_column_model()->get_default_order_column() + ($this->has_form_actions() ? 1 : 0), $this->get_default_row_count(), $this->get_column_model()->get_default_order_direction());
	 ...
	 * @return MenuManagerTableDataProvider The data provider.
	 */
	 ...
	 * @param MenuManagerTableDataProvider $data_provider The data provider.
	 */
	 ...
	 * @return MenuManagerTableColumnModel The column model.
	 */
	 ...
	 * @param MenuManagerTableColumnModel $model The column model.
	 */
	 ...
	 * @return MenuManagerTableCellRenderer The cell renderer.
	 */
	 ...
	 * @param MenuManagerTableCellRenderer $renderer The cell renderer.
	 */
	 ...
	 */
	function get_menu_items($offset, $count, $order_column, $order_direction)
	{
		$objects = $this->get_data_provider()->get_menu_items($offset, $count, $this->get_column_model()->get_column($order_column - ($this->has_form_actions() ? 1 : 0))->get_menu_property(), $order_direction);
		$table_data = array ();
		$column_count = $this->get_column_model()->get_column_count();
	...
	{
		return $this->get_data_provider()->get_menu_items_count();
	}
}
?>