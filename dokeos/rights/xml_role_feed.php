...
function dump_tree($roles)
{
	...
	echo '<node id="" class="type_category unlinked" title="', Translation :: get('Roles'), '">', "\n";
	
	while ($role = $roles->next_result())
	{
		$value = RightsUtilities :: role_for_element_finder($role);
		echo '<leaf id="', $role->get_id(), '" class="', $value['class'], '" title="', htmlentities($value['title']), '" description="', htmlentities($value['description']), '"/>', "\n";
	}
