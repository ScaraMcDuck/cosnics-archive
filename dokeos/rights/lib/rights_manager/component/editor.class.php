	...
	function get_rights_table_html()
	{
		...
		while ($role = $roles->next_result())
		{
			...
			foreach ($rights_array as $id => $name)
			{
				...
				$html[] = '<a href="'. $this->get_url(array(RightsManager :: PARAM_COMPONENT_ACTION => 'edit', 'role_id' => $role->get_id(), 'right_id' => $id, 'location_id' => $location_id)) .'">' . ($value == 1 ? '<img src="'. Theme :: get_common_img_path() .'action-setting-true.png" />' : '<img src="'. Theme :: get_common_img_path() .'action-setting-false.png" />') . '</a>';
				$html[] = '</div>';
			}
