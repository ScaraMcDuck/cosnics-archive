...
function create_document_link($www,$title,$path,$filetype,$size,$visibility)
{
	...
	$force_download_html = ($size==0)?'':'<a href="'.$forcedownload_link.'" style="float:right"'.$prevent_multiple_click.'><img src="../img/'.$forcedownload_icon.'" alt="" /></a>';
...
function build_document_icon_tag($type,$path)
{
	...
	return '<img src="../img/'.$icon.'" border="0" hspace="5" align="middle" alt="" />';
}
 ...
 */
function build_edit_icons($curdirpath,$type,$path,$visibility,$id, $is_allowed, $group_member_with_upload_rights)
{
	if(isset($_SESSION['_gid']))
	{
		$req_gid = '&amp;gidReq='.$_SESSION['_gid'];
	...
	if ($is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights) $modify_icons = '<a href="edit_document.php?curdirpath='.$curdirpath.'&amp;file='.urlencode($path).$gid_req.'"><img src="../img/edit.gif" border="0" title="'.get_lang('Modify').'" alt="" /></a>';
	if ($is_allowed[DELETE_RIGHT] || $group_member_with_upload_rights) $modify_icons .= '<a href="'.$_SERVER['PHP_SELF'].'?curdirpath='.$curdirpath.'&amp;delete='.urlencode($path).$gid_req.'&amp;'.$sort_params.'" onclick="return confirmation(\''.basename($path).'\');"><img src="../img/delete.gif" border="0" title="'.get_lang('Delete').'" alt="" /></a>';
	if ($is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights)  $modify_icons .= '<a href="'.$_SERVER['PHP_SELF'].'?curdirpath='.$curdirpath.'&amp;move='.urlencode($path).$gid_req.'"><img src="../img/move.gif" border="0" title="'.get_lang('Move').'" alt="" /></a>';
	if ($is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights)  $modify_icons .= '<a href="'.$_SERVER['PHP_SELF'].'?curdirpath='.$curdirpath.'&amp;'.$visibility_command.'='.$id.$gid_req.'&amp;'.$sort_params.'"><img src="../img/'.$visibility_icon.'.gif" border="0" title="'.get_lang('Visible').'" alt="" /></a>';
	return $modify_icons;
}
