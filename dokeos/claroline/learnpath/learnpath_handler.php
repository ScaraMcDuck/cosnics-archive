...
//block illegal use of script
if (! isset($_GET['learnpath_id']) && ! isset($_REQUEST['action']))
{
	api_not_allowed();
}
...
if ( (! $is_allowed_to_edit) or (! $is_allowed[EDIT_RIGHT]) )
{
	$htmlHeadXtra[] =  "<script type='text/javascript'> window.location=\"showinframes.php?source_id=5&action=$action&learnpath_id=$learnpath_id&chapter_id=$chapter_id&originalresource=no\"; </script>";
}
...
$interbredcrump[]= array ("url"=>$_SERVER['PHP_SELF']."?learnpath_id=$learnpath_id", "name" => "{$therow['learnpath_name']}");
