...
$location_id = RolesRights::get_course_tool_location_id($course_id, TOOL_LEARNPATH);
...
$dbTable                = Database::get_course_scormdocument_table();
...
$is_allowedToUnzip = ($is_allowed[EDIT_RIGHT] || $is_allowed[ADD_RIGHT]);

if($is_allowed[EDIT_RIGHT] || $is_allowed[ADD_RIGHT]) // for teacher only
{
...
if($is_allowed[EDIT_RIGHT]) // TEACHER ONLY
{
...
*/
//Introduction section
Display::display_introduction_section(TOOL_LEARNPATH, $is_allowed);
...
if($is_allowed[EDIT_RIGHT])
{
 /*--------------------------------------
	...
	$is_allowed[EDIT_RIGHT] ? $colspan = 9 : $colspan = 3;
	...
	if (!$curDirPath) {
		...
		while ($row=mysql_fetch_array($result)) {
			...
			if (($is_allowed[EDIT_RIGHT]) or ((!$is_allowed[EDIT_RIGHT]) and ($row3["visibility"] == '1'))) {
			...
			if($is_allowed[EDIT_RIGHT]) {
				...
				if ($is_allowed[DELETE_RIGHT]) echo "<a href='".$_SERVER['PHP_SELF']."?action=deletepath&id=".$row["learnpath_id"]."'><img src=\"../img/delete.gif\" border=\"0\" title=\"".get_lang('_delete_learnpath')."\" onClick=\"return confirmation('".$row2['learnpath_name']."');\"></a>";
