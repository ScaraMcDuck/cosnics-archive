...
$location_id = RolesRights::get_course_tool_location_id($course_id, TOOL_CALENDAR_EVENT);
...
Display::display_introduction_section(TOOL_CALENDAR_EVENT, $is_allowed);
...
{ 
	...
	$agenda_items=get_kalender_items($select_month, $select_year, $is_allowed);
...
if ($is_allowed[ADD_RIGHT] || $is_allowed[EDIT_RIGHT] || $is_allowed[DELETE_RIGHT])
{
	switch ($_GET['action'])
	{
		case "add":
			...
			if ($_POST['submit_event'])
			{
				...
				display_agenda_items($is_allowed);
			}
			else
			{
				show_add_form();
		...
		case "delete":
			$id=(int)$_GET['id'];
			delete_agenda_item($id, $is_allowed);
			display_agenda_items($is_allowed);
			break;
