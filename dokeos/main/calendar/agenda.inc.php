...
* @return array
*/
function get_kalender_items($month, $year, $is_allowed)
{
...
{
	echo "<li><a href='".$_SERVER['PHP_SELF']."?action=add&amp;origin=".$_GET['origin']."'><img src=\"../img/calendar_add.gif\" border=\"0\" /> ".get_lang("AgendaAdd")."</a><br /></li>";
...
*/
function delete_agenda_item($id, $is_allowed)
{
...
* @author Patrick Cool <patrick.cool@UGent.be>, Ghent University
*/
function display_agenda_items($is_allowed)
{
	global $TABLEAGENDA;
	...
	while ($myrow=mysql_fetch_array($result))
	{
		if (!in_array($myrow['id'], $displayed))	
		{
			...
			*/
			...
			{
				// edit
				if ($is_allowed[EDIT_RIGHT]) echo "<a href=\"".$_SERVER['PHP_SELF']."?origin=".$_GET['origin']."&amp;action=edit&amp;id=".$myrow['id']."\">",
						"<img src=\"../img/edit.gif\" border=\"0\" alt=\"".get_lang("ModifyCalendarItem")."\" /></a>";
				if ($is_allowed[DELETE_RIGHT]) echo "<a href=\"".$_SERVER['PHP_SELF']."?origin=".$_GET['origin']."&amp;action=delete&amp;id=".$myrow['id']."\" onclick=\"javascript:if(!confirm('".addslashes(htmlentities(get_lang("ConfirmYourChoice")))."')) return false;\">",
...
<table border="0" cellpadding="5" cellspacing="0" width="100%" id="newedit_form">
	...
	<?php /* This has been replaced by dividing it into date and hour. It saves a line
	<tr class="subtitle">
			...
			<select name="fday" onchange="javascript:document.new_calendar_item.end_fday.value=this.value;">
				<?php
					...
					foreach (range(1, 31) as $i)
					{
						...
						if ($value==$day)
							echo "\t\t\t\t<option value=\"$value\" selected=\"selected\">$i</option>\n";
						else
	...
	<tr class="subtitle">
		<td>
			...
			<select name="end_fyear">
				...
				<option value="<?php echo $end_year ?>" selected="selected"><?php echo $end_year ?></option>
				<?php
					echo "\n";
					for ($i=1; $i<=5; $i++)
					{
