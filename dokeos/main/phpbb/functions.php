...
function get_whosonline($IP, $username, $forum, $db)
{
	...
	if($userlog == false)
	{
		$ok= @mysql_query( "INSERT INTO `$tbl_whosonline`
		                    (ID,IP,online_date,username,forum)
		                    VALUES
...
function bbencode_list($message)
{
	...
	while ($curr_pos && ($curr_pos < strlen($message)))
	{
		...
		if ($curr_pos)
		{
			...
			$possible_end = substr($message, $curr_pos, 7);
			$matches = array(); // might be passed to preg_match()
...
function make_phpbb_clickable($text)
{
...
/**
 * Reverses the effects of make_phpbb_clickable(), for use in editpost.
...
function error_die($msg)
{
	...
	global $starttime, $langManager, $langPlatform;
