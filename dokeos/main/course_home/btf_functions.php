...
function showtools2($cat, $role_id, $is_allowed)
{
	GLOBAL $_user;

	$dokeosRepositoryWeb = api_get_path(WEB_CODE_PATH);
	$TBL_ACCUEIL = Database :: get_course_tool_list_table();
	$TABLE_TOOLS = Database :: get_main_table(MAIN_COURSE_MODULE_TABLE);

	$role_id = isset ($_SESSION['view_as_role']) ? $_SESSION['view_as_role'] : $role_id;
	if ($role_id == TEACHING_ASSISTANT || $role_id == COURSE_ADMIN)
		$role_id = NORMAL_COURSE_MEMBER;
	...
	foreach ($toolsRow_all as $toolsRow)
	{
		...
		{
			$toolsRow['link'] = $dokeosRepositoryWeb.$toolsRow['link'];
		...
		$toolsRow['img'] = $dokeosRepositoryWeb."img/".$toolsRow['img'];

		//If the current tool isn't an external link, set visibility matching the current role
		if ($toolsRow['img'] == 'external.gif')
		{
			$tool_location = RolesRights :: get_course_tool_location_id($_SESSION['_course']['sysCode'], $toolsRow['name']);
			$toolsRow['visibility'] = RolesRights :: is_allowed($role_id, VIEW_RIGHT, $tool_location);
		}
		...
		$lnk = array ();
		if ($is_allowed[EDIT_RIGHT] and $cat != "courseAdmin" && !strpos($toolsRow['link'], 'learnpath_handler.php?learnpath_id'))
		{
			...
			if (is_array($lnk))
			{
				foreach ($lnk as $thisLnk)
				{
					...
					{
						$cell_content .= '<a href="'.$properties['adminlink'].'">'.'<img src="'.api_get_path(WEB_CODE_PATH).'/img/edit.gif" alt="'.get_lang('Edit').'"/>'.'</a>';
