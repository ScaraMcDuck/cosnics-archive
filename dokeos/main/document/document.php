...
if(isset($_SESSION['_gid']) && $_SESSION['_gid']!='') 
{
	...
	{
		if($is_allowed[EDIT_RIGHT] || GroupManager::is_subscribed($_uid,$_SESSION['_gid'])) //only courseadmin or group members allowed
		{
	...
	elseif($group_properties['doc_state']==1)  //documents are public
	{
		...
		$interbredcrump[]= array ("url"=>"../group/group_space.php?gidReq=".$_SESSION['_gid'], "name"=> get_lang('GroupSpace'));
		//allowed to upload?
		if($is_allowed[ADD_RIGHT] || GroupManager::is_subscribed($_uid,$_SESSION['_gid'])) //only courseadmin or group members can upload
		{
			$group_member_with_upload_rights = true;
		}
	}
...
if($is_allowed_to_edit || $is_allowed[ADD_RIGHT] || $is_allowed[EDIT_RIGHT])
{
...
$location_id = RolesRights::get_course_tool_location_id($course_id, TOOL_DOCUMENT);
...
Display::display_introduction_section(TOOL_DOCUMENT, $is_allowed);

/*============================================================================*/

if($is_allowed[EDIT_RIGHT] || $is_allowed[ADD_RIGHT] || $is_allowed[DELETE_RIGHT] || $group_member_with_upload_rights) // TEACHER ONLY
{
	...
	if (isset($_GET['move']) && $_GET['move']!='')
	{
		...
		{
			$access_all_right = $is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights;
			$folders = DocumentManager::get_all_document_folders($_course,$to_group_id,$access_all_right);
...
$allow_all_access = $is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights;
$docs_and_folders = DocumentManager::get_all_document_data($_course,$curdirpath,$to_group_id,NULL,$allow_all_access);
...
<?php 
$is_allowed_all_access = $is_allowed[EDIT_RIGHT] || $group_member_with_upload_rights;
$folders = DocumentManager::get_all_document_folders($_course,$to_group_id,$is_allowed_all_access);
...
?>
	...
	if ($is_allowed[ADD_RIGHT] || $group_member_with_upload_rights)
	{
...
if($docs_and_folders)
{
	...
	while (list ($key, $id) = each($docs_and_folders))
	{
		...
		$size = $id['filetype']=='folder' ? get_total_folder_size($id['path'],$is_allowed[EDIT_RIGHT]) : $id[size];
		//get the title or the basename depending on what we're using
		$document_name = ($use_document_title=='true')?$id['title']:basename($id['path']);
		//$row[] = $key; //testing
		
		//data for checkbox
		if ($is_allowed[DELETE_RIGHT] || $group_member_with_upload_rights)
		{
			$row[] = $id['path'];
		}
		...
		if ($is_allowed[EDIT_RIGHT] || $is_allowed[DELETE_RIGHT] || $group_member_with_upload_rights)
		{
			$row[] = build_edit_icons($curdirpath,$id['filetype'],$id['path'],$id['visibility'],$key, $is_allowed, $group_member_with_upload_rights);
		}
		$sortable_data[] = $row;
	}
...
==============================================================================
*/ 
if ($is_allowed[EDIT_RIGHT]) display_document_options();
