...
$is_allowed_to_edit = $is_allowed[EDIT_RIGHT];
...
if(isset($_SESSION['_gid']) && $_SESSION['_gid']!='') //if the group id is set, check if the user has the right to be here
{
	...
	if($is_allowed[ADD_RIGHT] || GroupManager::is_subscribed($_uid,$_SESSION['_gid'])) //only courseadmin or group members allowed
	{
...
elseif($is_allowed[ADD_RIGHT]) //admin for "regular" upload, no group documents
{
	$to_group_id = 0;
	$req_gid = '';
}
else  //not allowed to add files
{
	api_not_allowed();
}
...
$interbredcrump[]=array("url"=>"./document.php?curdirpath=".urlencode($path).$req_gid, "name"=> $langDocuments);
