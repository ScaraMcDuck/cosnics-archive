...
{
	echo '<p><a href="?action=add">'.get_lang('AddAnnouncement').'</a></p>';
}
$form_action = "";
...
if ($action_todo)
{
	...
	if($form->validate())
	{
		...
		{
			case 'add':
				SystemAnnouncementManager::add_announcement($values['title'],$values['content'],$values['start'],$values['end'],$values['visible_teacher'],$values['visible_student'],$values['visible_guest']);
