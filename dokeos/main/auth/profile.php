...
function upload_user_image($_uid)
{
	...
	$image_repository = api_get_code_sys_path().'upload/users/';
...
function build_production_list($_uid, $force = false)
{
	...
	$production_dir = api_get_code_web_path()."upload/users/$_uid/";
	$del_image = api_get_code_web_path().'img/delete.gif';
...
{
	$production_repository = api_get_code_sys_path()."upload/users/$_uid/";
	$productions = array();
...
{
	unlink(api_get_code_sys_path()."upload/users/$_uid/$production");
}
...
//	USER PICTURE
$image = get_user_image($_uid);
$image_file = ($image != '' ? api_get_code_web_path()."upload/users/$image" : api_get_code_web_path().'img/unknown.jpg');
...
echo '<div style="clear:both; border-top:thin solid; padding-top:2px;">
	<a href="'.api_get_code_web_path().'tracking/personnalLog.php">'.get_lang('MyStats').'</a>
	| <a href="'.api_get_code_web_path().'auth/courses.php">'.get_lang('MyCourses').'</a></div>';
