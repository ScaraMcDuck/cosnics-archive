...
if ($_GET["source_id"])
{
	...
	if ($from_learnpath != 'yes')
	{
		if (!$_SESSION["origintoolurl"] OR $_SESSION["origintoolurl"]<>$interbredcrump["url"])
		{
...
if ($from_learnpath != 'yes')
{
	...
	$interbredcrump[] = array ("url" => $_SESSION["origintoolurl"], "name" => $_SESSION["origintoolname"]);
}
else
{
	...
	$interbredcrump[] = array ("url" => "../scorm/scormdocument.php", "name" => get_lang('LearningPath'));
	$interbredcrump[] = array ("url" => "../learnpath/learnpath_handler.php?learnpath_id=$learnpath_id", "name" => "{$therow['learnpath_name']}");
	$interbredcrump[] = array ("url" => $_SERVER['PHP_SELF']."?action=$action&learnpath_id=$learnpath_id&chapter_id=$chapter_id&originalresource=no", "name" => "{$therow2['chapter_name']}");
