...
{
	/**
	* Set the correct visibility rights to show/hide the specified course tool to
	* normal course members (and guests, depending on the course visibility settings).
	*/
	function set_course_tool_visibility($tool_table_id, $visibility)
	{
		//retrieve tool name constant from course tool table
		$tool_table = Database :: get_course_tool_list_table($database_name);
		$sql_query = "SELECT name FROM $tool_table WHERE id='$tool_table_id'";
		$sql_result = api_sql_query($sql_query, __FILE__, __LINE__);
		$result = mysql_fetch_array($sql_result);
		$tool_name = $result['name'];

		//get location id based on tool name
		$course_code = api_get_course_id();
		$course_location_id = RolesRights :: get_course_location_id($course_code);
		$tool_location_id = RolesRights :: get_course_tool_location_id($course_code, $tool_name);

		//adjust role-right-location table
		$allowed_for_anonymous = RolesRights :: is_allowed(ANONYMOUS_GUEST_COURSE_VISITOR, VIEW_RIGHT, $course_location_id);
		$allowed_for_registered_guest = RolesRights :: is_allowed(REGISTERED_GUEST_COURSE_VISITOR, VIEW_RIGHT, $course_location_id);
		$allowed_for_normal_member = RolesRights :: is_allowed(NORMAL_COURSE_MEMBER, VIEW_RIGHT, $course_location_id);

		if ($allowed_for_anonymous)
			RolesRights :: set_value(ANONYMOUS_GUEST_COURSE_VISITOR, VIEW_RIGHT, $tool_location_id, $visibility);
		if ($allowed_for_registered_guest)
			RolesRights :: set_value(REGISTERED_GUEST_COURSE_VISITOR, VIEW_RIGHT, $tool_location_id, $visibility);
	...
	*
	* Exception: for teaching assistants and course admins, we return a list of tools visible
	* for normal course members. The tools not visible for normal course members are visible to course
	* admins but in other sections (admin only, public but hidden...)
	*/
	function get_accessible_public_tools($course_id, $role_id)
	{
	...
	*/
	function get_hidden_public_tools($course_id, $role_id)
	{
		//get list of tools of the current course
		$course_tool_table = Database::get_course_tool_list_table();
	...
	* This section is normally only visible for the course admin, who can make tools visible for others there.
	*/
	function get_course_admin_section_tools($course_id, $role_id)
	{
		//get list of tools of the current course
		$course_tool_table = Database::get_course_tool_list_table();
	...
	/**
	* Returns a list of tools only visible for the platform admin.
	*/
	function get_platform_admin_section_tools($course_id, $role_id)
	{
		//get list of tools of the current course
		$course_tool_table = Database::get_course_tool_list_table();
	...
	* The user starts by default with student rights in the course,
	* or you can optionally pass a $status (COURSEMANAGER|STUDENT) parameter
	*
	...
	* @todo replace magic constants for role id
	*/
	...
	*/
	function is_user_subscribed_in_real_or_linked_course($user_id, $course_code)
	{
		$course_table = Database :: get_main_table(MAIN_COURSE_TABLE);
		$course_user_table = Database :: get_main_table(MAIN_COURSE_USER_TABLE);
	...
	*/
	...
	function delete_course($code)
	{
		...
		foreach ($location_list as $this_location)
		{
			...
			$remove_sql = "DELETE FROM $role_right_location_table WHERE location_id='$location_id'";
			api_sql_query($remove_sql, __FILE__, __LINE__);
			$remove_sql = "DELETE FROM $location_table WHERE id='$location_id'";
			api_sql_query($remove_sql, __FILE__, __LINE__);
		}
	}
