...
function my_rename($filePath, $newFileName)
{
	...
	return rename($oldFileName, $newFileName) ? $newFileName : false;

	/* PREVIOUS VERSION OF THE CODE

	WARNING (Roan)
	$baseWorkDir is used but is empty here.
	so filePath is indeed $filePath (string) - complete path of the file or the directory
	...
	if ((!ereg("[[:print:]]+\.[[:alnum:]]+$", $newFileName))
		&& ereg("[[:print:]]+\.([[:alnum:]]+)$", $oldFileName, $extension))
	{
	...
	$newFileName = replace_dangerous_char($newFileName,'strict');
