...
function update_user_course_properties($user_id, $course_code, $properties)
{
	...
	$result = api_sql_query("UPDATE $tbl_coursUser
	                        SET     `role`       = '".$properties['description']."',
	                                ".$sqlChangeStatus."
	...
	$new_role_id = $properties['local_role'];
	$location = RolesRights::get_short_course_location_path($course_code);
	$location_id = RolesRights::get_course_location_id_from_short_location($location);
	RolesRights::set_user_local_role($user_id, $new_role_id, $location_id);
