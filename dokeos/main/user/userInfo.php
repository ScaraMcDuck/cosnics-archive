...
if ($allowedToEditDef)
{
	...
	elseif ($submitMainUserInfo)
	{
		...
		$userProperties['description'] = $role; //the old freeform role field
		$userProperties['local_role'] = $_POST['local_role'];
...
elseif ($displayMode == "viewMainInfoEdit")
{
	...
	if ($mainUserInfo)
	{
		...
		echo "<td>". get_lang('Role') . "</td>"; //for new roles and rights system
		echo "<td>", get_lang('Tutor'), "</td>\n", "<td>", get_lang('CourseManager'), "</td>\n", "</tr>\n" ;
		 
		 //user info data
		echo "<tr align=\"center\">", "<td align=\"left\"><b>", htmlize($mainUserInfo['firstName']), " ", htmlize($mainUserInfo['lastName']), "</b></td>\n", "<td align=\"left\"><input type=\"text\" name =\"role\" value=\"", $mainUserInfo['role'], "\" maxlength=\"40\"></td>";
		
		$local_role_list = RolesRights::get_local_role_list();
		echo "<td><select name=\"local_role\">\n";
		foreach ($local_role_list as $local_role)
		{
			echo "<option value=\"".$local_role["id"] . "\">" . RolesRights::get_visual_role_name_from_name($local_role["name"]) . "</option>\n";
		}
		echo "</select></td>\n";
...
elseif ($displayMode == "viewContentList") // default display
{
	...
	if ($mainUserInfo)
	{
		...
		$role_name = RolesRights::get_visual_local_user_role($userIdViewed, $course_id);
