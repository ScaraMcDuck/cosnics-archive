...
function mdo_add_breadcrump_nav()
{
    ...
    $interbredcrump[]= array ('url' => $docurl, 
    ...
    if (($docpath = $this->mdo_path))
    {
        ...
        if (strlen($docpath) > 1) $interbredcrump[]= array ('url' => 
    ...
    require_once(api_get_library_path() . '/xht.lib.php');
    require_once(api_get_library_path() . '/document.lib.php');
...
function mdobject($_course, $id)
{
            ...
            addslashes($id) . "'", __FILE__, __LINE__))))
    {
        ...
        $group_info = Database::get_course_group_table();
