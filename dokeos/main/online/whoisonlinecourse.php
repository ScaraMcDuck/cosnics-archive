...
if ($_GET["chatid"] != '') {
	...
	$chatid = $_GET['chatid'];
	$sql="update $track_user_table set chatcall_user_id = '$_uid', chatcall_date = '$time', chatcall_text = '' where (user_id = $chatid)";
...
if ((get_setting('showonline','course') == "true" AND $_course))
{
	$userlist = Who_is_online_in_this_course($_uid,$MINUTE,$_course['id']);
	...
	//	if (!IsValidUser($_GET["id"],$mysqlMainDb))
		if ($_GET["id"]=='')
		{
					...
					//$row[0] - the user_id ; $row[1] - last login date
				{
					...
					$uid=$row[0];
					$name=GetFullUserName($row[0],$mysqlMainDb).($_uid==$row[0]?("&nbsp;<b>(".get_lang('Me').")</b>"):(""));
	
					//$row[1] is the last click timestamp
					$sql="select * from $track_user_table where ( user_id = $uid )";
					$result=api_sql_query($sql,__FILE__,__LINE__);
					$row2=mysql_fetch_array($result);
					if ($row2['status']==1) { $status=get_lang('Teacher'); } else { $status=get_lang('Student'); }
		...
		{
					$name=GetFullUserName($_GET["id"],$mysqlMainDb).($_uid==$_GET["id"]?("&nbsp;<b>(".get_lang('Me').")</b>"):(""));
					$alt=GetFullUserName($_GET["id"],$mysqlMainDb).($_uid==$_GET["id"]?("&nbsp;(".get_lang('Me').")"):(""));
					//$row[1] is the last click timestamp
					$sql="select * from $track_user_table where ( user_id = ".$_GET["id"]." )";
					$result=api_sql_query($sql,__FILE__,__LINE__);
					$row2=mysql_fetch_array($result);
					...
					if ($row2['picture_uri']<>'') {
						...
						echo "<tr><td colspan=3 align=center><img src=$fullurl alt=\"".$alt."\"></td></tr><tr><td>&nbsp;</td></tr>";
					}
...
echo "<table width=100%>";
echo "<tr><td align=left>&nbsp;<A HREF=\"javascript:history.back()\">< ".get_lang('Back')."</A></td>";
