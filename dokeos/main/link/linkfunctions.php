...
function showlinksofcategory($catid)
{
	...
	while ($myrow = mysql_fetch_array($result))
	{
		...
		{
			echo "<br/>";
			if ($is_allowed[EDIT_RIGHT]) echo "<a href=\"".$_SERVER['PHP_SELF']."?action=editlink&amp;category=$category&amp;id=$myrow[0]&amp;urlview=$urlview\">", "<img src=\"../img/edit.gif\" border=\"0\" alt=\"", get_lang('Modify'), "\"/>", "</a>";
			if ($is_allowed[DELETE_RIGHT]) echo " <a href=\"".$_SERVER['PHP_SELF']."?action=deletelink&amp;id=", $myrow[0], "&amp;urlview=", $urlview, "\" onclick=\"javascript:if(!confirm('".get_lang('LinkDelconfirm')."')) return false;\">", "<img src=\"../img/delete.gif\" border=\"0\" alt=\"", get_lang('Delete'), "\"/>", "</a>";
