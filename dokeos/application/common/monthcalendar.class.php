...
class MonthCalendar extends CalendarTable
{
	...
	public function add_calendar_navigation($url_format)
	{
		...
		$navigation->setCellContents(0, 0, '<a href="'.str_replace('-TIME-', $prev, $url_format).'"><img src="'.Theme :: get_common_img_path().'action-prev.png" style="vertical-align: middle;" alt="&lt;&lt;"/></a> ');
		$navigation->setCellContents(0, 1, Translation :: get(date('F',  $this->get_display_time()).'Long').' '.date('Y',  $this->get_display_time()));
		$navigation->setCellContents(0, 2, ' <a href="'.str_replace('-TIME-', $next, $url_format).'"><img src="'.Theme :: get_common_img_path().'action-next.png" style="vertical-align: middle;" alt="&gt;&gt;"/></a> ');
