...
class SurveyPublisher extends SurveyPublisherComponent
{
	...
	function run()
	{
		...
		$wdm = AssessmentDataManager :: get_instance();
		
		$pid = Request :: get(AssessmentManager :: PARAM_ASSESSMENT_PUBLICATION);
		$publication = $wdm->retrieve_assessment_publication($pid);
		$survey = $publication->get_publication_object();
		
		$form = new SurveyPublicationForm($this->parent, $survey, $this->parent->get_url(array(AssessmentManager :: PARAM_ASSESSMENT_PUBLICATION => $pid)));
		
		if ($form->validate())
		{
			$values = $form->exportValues();
			$this->parse_values($values, $survey, $pid);
			$this->parent->redirect(Translation :: get('UsersInvited'), false, array(self :: PUBLISH_ACTION => self :: ACTION_VIEW, AssessmentManager :: PARAM_ASSESSMENT_PUBLICATION => $pid));
		}
		else
		{
	...
	function create_invitation($user, $mail, $survey, $resend, $pid)
	{
		...
		$invitations = AssessmentDataManager :: get_instance()->retrieve_survey_invitations($condition);
	...
	{
		$url = $this->parent->get_url(array(AssessmentManager :: PARAM_ACTION => AssessmentManager :: ACTION_VIEW_ASSESSMENT_PUBLICATION, AssessmentManager :: PARAM_INVITATION_ID => $survey_invitation->get_invitation_code()));
