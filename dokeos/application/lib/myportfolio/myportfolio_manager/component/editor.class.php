...
{	
	...
	function run()
	{
		...
		if ($item >= -1)
		{
			...
			{
				//$this->redirect(MyPortfolio :: ACTION_VIEW, get('EditNotAllowed'), $object->get_parent_id(), true);
			}
			*/
			$form = LearningObjectForm :: factory(LearningObjectForm :: TYPE_EDIT, $object, 'edit', 'post', $this->get_url(array (MyPortfolio :: PARAM_ITEM => $item)));
			if ($form->validate())
			{
				$success = $form->update_learning_object();
				//$category_id = $object->get_parent_id();
				//$this->redirect(MyPortfolio :: ACTION_VIEW, Translation :: get($success == LearningObjectForm :: RESULT_SUCCESS ? 'ObjectUpdated' : 'ObjectUpdateFailed'), $category_id);
				$this->redirect(null, Translation :: get($success == LearningObjectForm :: RESULT_SUCCESS ? 'ObjectUpdated' : 'ObjectUpdateFailed'), false, array(MyPortfolio :: PARAM_ACTION => MyPortfolio :: ACTION_VIEW, MyPortfolio :: PARAM_ITEM => $item));
			}
			else
			{
	...
	function get_editor_as_html()
	{
		...
		if (true)
		{
			...
			elseif (!$object->is_latest_version())
			{
				$this->redirect(MyPortfolio :: ACTION_VIEW, Translation :: get('EditNotAllowed'), $object->get_parent_id(), true);
			}
			$form = LearningObjectForm :: factory(LearningObjectForm :: TYPE_EDIT, $object, 'edit', 'post', $this->get_url(array (MyPortfolio :: PARAM_ITEM => $id)));
			if ($form->validate())
			{
				$success = $form->update_learning_object();
				//$category_id = $object->get_parent_id();
				//$this->redirect(MyPortfolio :: ACTION_VIEW, get($success == LearningObjectForm :: RESULT_SUCCESS ? 'ObjectUpdated' : 'ObjectUpdateFailed'), $category_id);
				$this->redirect(null, Translation :: get($success == LearningObjectForm :: RESULT_SUCCESS ? 'ObjectUpdated' : 'ObjectUpdateFailed'), false, array(MyPortfolio :: PARAM_ACTION => MyPortfolio :: ACTION_VIEW, MyPortfolio :: PARAM_ITEM => $id));
			}
			else
			{
				$breadcrumbs = array(array('url' => $this->get_url(), 'name' => Translation :: get('Edit')));
