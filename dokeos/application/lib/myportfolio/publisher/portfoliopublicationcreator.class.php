...
class PortfolioPublicationCreator extends PortfolioPublisherComponent
{
	...
	private function get_editing_form($objectID)
	{
		...
		$form = LearningObjectForm :: factory(LearningObjectForm :: TYPE_REPLY, $object, 'edit', 'post', $this->get_url(array (PortfolioPublisher :: PARAM_LEARNING_OBJECT_ID => $objectID, PortfolioPublisher :: PARAM_EDIT => 1)));
		if ($form->validate())
		{
			$object = $form->create_learning_object();
			return $this->get_publication_form($object->get_id(), true);
