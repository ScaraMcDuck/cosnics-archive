...
class PortfolioPublisher
{
	...
	function as_html()
	{
		...
		foreach (array ('publicationcreator','browser', 'finder') as $action)
		{
			...
			$out .= ' href="'.$this->get_url(array (PortfolioPublisher :: PARAM_ACTION => $action), true).'">'.htmlentities(Translation :: get_lang(ucfirst($action).'Title')).'</a></li>';
		}
		$out .= '</ul><div class="tabbed-pane-content">';
		$action = $this->get_action();
		require_once dirname(__FILE__).'/publisher/portfolio'.$action.'.class.php';
