...
class DatabasePersonalCalendarDatamanager extends PersonalCalendarDatamanager
{
	...
	{
		return $this->database->escape_column_name($name);
	}
	...
	public function get_learning_object_publication_attributes($object_id, $type = null, $offset = null, $count = null, $order_property = null, $order_direction = null)
	{
		if (isset($type))
		{
			if ($type == 'user')
			{
				$query = 'SELECT * FROM '.$this->database->get_table_name('publication').' WHERE '.$this->database->escape_column_name('publisher').'=?';
		...
		{
			$query = 'SELECT * FROM '.$this->database->get_table_name('publication').' WHERE '.$this->database->escape_column_name('calendar_event').'=?';
			$statement = $this->database->get_connection()->prepare($query);
	...
	public function count_publication_attributes($type = null, $condition = null)
	{
		...
		return $this->database->count_objects('publication', $condition);
	}
	/**
	...
	public function delete_learning_object_publications($object_id)
	{
		...
		$this->database->delete('publication',$condition);
	}
	/**
    ...
    function retrieve_shared_calendar_event_publications($condition = null, $orderBy = array (), $orderDir = array (), $offset = 0, $maxObjects = -1)
	{
		...
		$query = 'SELECT DISTINCT pub.* FROM '.$this->database->escape_table_name('publication') . ' as pub';
		$query .= ' LEFT JOIN '.$this->database->escape_table_name('publication_user') . ' ON pub.id = '.$this->database->escape_table_name('publication_user').'.publication';
		$query .= ' LEFT JOIN '.$this->database->escape_table_name('publication_group') . ' ON pub.id = '.$this->database->escape_table_name('publication_group').'.publication';
	...
	function update_calendar_event_publication_id($publication_attr)
	{
		...
		$this->database->get_connection()->loadModule('Extended');
		if ($this->database->get_connection()->extended->autoExecute($this->database->get_table_name('publication'), $props, MDB2_AUTOQUERY_UPDATE, $where))
		{
			return true;
		}
		else
