...
{	
	...
	function run()
	{
		...
		{
			$this->display_error_page(htmlentities(get_lang('NoProfileSelected')));
		}
	}
	...
	function get_publication_as_html()
	{
		...
		$html[] = '<div class="learning_object" style="background-image: url('.api_get_path(WEB_CODE_PATH).'img/profile.gif);">';
		...
		if ($message->supports_attachments())
		{
			...
			if (count($attachments))
			{
				...
				{
					$html[] = '<li class="profile_attachment"><div style="float: left;"><img src="'.api_get_path(WEB_CODE_PATH).'/img/treemenu_types/'.$attachment->get_type().'.gif" alt="'.htmlentities(get_lang(LearningObject :: type_to_class($attachment->get_type()).'TypeName')).'"/></div><div style="float: left;">&nbsp;'.$attachment->get_title().'&nbsp;</div>';
