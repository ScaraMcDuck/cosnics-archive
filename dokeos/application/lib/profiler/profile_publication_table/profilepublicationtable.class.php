...
class ProfilePublicationTable
{
	 ...
	 * Default table name
	 */
	const DEFAULT_NAME = 'profile_publication';
	/**
	 ...
	 *                                                       default renderer.
	 */
	function ProfilePublicationTable($data_provider, $table_name = null, $column_model = null, $cell_renderer = null)
	{
		$this->set_data_provider($data_provider);
		$this->set_name(isset($table_name) ? $table_name : self :: DEFAULT_NAME);
		$this->set_column_model(isset ($column_model) ? $column_model : new DefaultProfilePublicationTableColumnModel());
		$this->set_cell_renderer(isset ($cell_renderer) ? $cell_renderer : new DefaultProfilePublicationTableCellRenderer());
	...
	function as_html()
	{
		$table = new SortableTable($this->get_name(), array ($this, 'get_profile_publication_count'), array ($this, 'get_profile_publications'), $this->get_column_model()->get_default_order_column() + ($this->has_form_actions() ? 1 : 0), $this->get_default_row_count(), $this->get_column_model()->get_default_order_direction());
	 ...
	 */
	function get_profile_publications($offset, $count, $order_column, $order_direction)
	{
		$objects = $this->get_data_provider()->get_profile_publications($offset, $count, $this->get_column_model()->get_column($order_column - ($this->has_form_actions() ? 1 : 0))->get_profile_property(), $order_direction);
		$table_data = array ();
		$column_count = $this->get_column_model()->get_column_count();
	...
	{
		return $this->get_data_provider()->get_profile_publication_count();
	}
}
?>