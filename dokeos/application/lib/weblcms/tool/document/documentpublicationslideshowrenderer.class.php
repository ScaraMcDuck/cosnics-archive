...
class DocumentPublicationSlideshowRenderer extends ListLearningObjectPublicationListRenderer
{
	function as_html()
	{
		...
		{
			$toolbar_data[] = array(
				'img'=>Theme :: get_common_img_path().'action-slideshow.png',
		...
		else
		{
			...
			$toolbar_data[] = array(
				'img'=>Theme :: get_common_img_path().'action-slideshow-thumbnails.png',
			...
			if (!$first)
			{
				...
				$navigation[] = '<a href="'.$this->get_url(array ('slideshow_index' => $slideshow_index -1)).'"><img src="'.Theme :: get_common_img_path().'action-prev.png" alt="'.Translation :: get('Previous').'"/></a>';
			}
			else
			{
				$navigation[] = '<img src="'.Theme :: get_common_img_path().'action-first-na.png" alt="'.Translation :: get('First').'"/>';
				$navigation[] = '<img src="'.Theme :: get_common_img_path().'action-prev-na.png" alt="'.Translation :: get('Previous').'"/>';
			}
			$navigation[] = '</div>';
			$navigation[] = '<div style="width=30%;text-align:right;float:right;">';
			if (!$last)
			{
				$navigation[] = '<a href="'.$this->get_url(array ('slideshow_index' => $slideshow_index +1)).'"><img src="'.Theme :: get_common_img_path().'action-next.png" alt="'.Translation :: get('Next').'"/></a>';
			...
			{
				$navigation[] = '<img src="'.Theme :: get_common_img_path().'action-next_na.png" alt="'.Translation :: get('Next').'"/>';
