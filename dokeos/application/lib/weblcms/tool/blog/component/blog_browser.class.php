...
require_once dirname(__FILE__).'/blog_publication_table/blog_publication_table.class.php';

class BlogToolBrowserComponent extends BlogToolComponent
{
	private $action_bar;
	...
	function run()
	{
		...
		$table = new BlogPublicationTable($this, $this->get_user(), array('blog'), null);
		echo $table->as_html();
	...
	function get_toolbar() 
	{
		...
		$action_bar->add_common_action(
			new ToolbarItem(
				Translation :: get('Publish'), Theme :: get_common_image_path().'action_publish.png', $this->get_url(array(BlogTool :: PARAM_ACTION => BlogTool :: ACTION_PUBLISH)), ToolbarItem :: DISPLAY_ICON_AND_LABEL
			)
		);
		
		$action_bar->add_common_action(
			new ToolbarItem(
				Translation :: get('Browse'), Theme :: get_common_image_path().'action_browser.png', $this->get_url(array(BlogTool :: PARAM_ACTION => BlogTool :: ACTION_BROWSE_BLOGS)), ToolbarItem :: DISPLAY_ICON_AND_LABEL
			)
		);
