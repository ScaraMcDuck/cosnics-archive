...
class GlossaryToolViewerComponent extends GlossaryToolComponent 
{
	...
	function run()
	{
		...
		else
		{
    		...
    		$children = $dm->retrieve_complex_learning_object_items(new EqualityCondition(ComplexLearningObjectItem :: PROPERTY_PARENT, Request :: get(Tool :: PARAM_PUBLICATION_ID)));
    		while($child = $children->next_result())
    		{
    			$lo = $dm->retrieve_learning_object($child->get_ref());
    			echo $this->display_learning_object($lo);
    		}
		}
	...
	function display_learning_object($lo)
	{
	...
	function get_actions($glossary_item) 
	{
		...
		{
			$actions[] = array(
				'href' => $this->get_url(array(Tool :: PARAM_ACTION => Tool :: ACTION_EDIT_CLOI, Tool :: PARAM_COMPLEX_ID => $glossary_item->get_id(), Tool :: PARAM_PUBLICATION_ID => Request :: get('pid'))),
		...
		{
			$actions[] = array(
				'href' => $this->get_url(array(Tool :: PARAM_ACTION => Tool :: ACTION_DELETE_CLOI, Tool :: PARAM_COMPLEX_ID => $glossary_item->get_id(), Tool :: PARAM_PUBLICATION_ID => Request :: get('pid'))),
