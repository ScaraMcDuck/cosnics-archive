	...
	function run()
	{
		...
		else
		{
			...
			else
			{
				...
				{
					echo '<a href="' . $this->get_url(array('admin' => 1), true) . '"><img src="'.$this->get_parent()->get_path(WEB_IMG_PATH).'publish.gif" alt="'.Translation :: get('Publish').'" style="vertical-align:middle;"/> '.Translation :: get('Publish').'</a> ';
					echo '<a href="' . $this->get_url(array('category_manager_action' => 1), true) . '"><img src="'.$this->get_parent()->get_path(WEB_IMG_PATH).'category.gif" alt="'.Translation :: get('ManageCategories').'" style="vertical-align:middle;"/> '.Translation :: get('ManageCategories').'</a></p>';
				}
