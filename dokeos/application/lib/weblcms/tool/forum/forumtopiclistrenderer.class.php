...
class ForumTopicListRenderer extends TableLearningObjectPublicationListRenderer
{
    function ForumTopicListRenderer($browser)
    {
		...
		$this->set_header($column++, get_lang('Replies'), false);
		$this->set_header($column++, get_lang('Author'), false);
    ...
    function render_lock_action($topic)
	{
		...
		$link = '<a href="'.$url.'" onclick="return confirm(\''.addslashes(htmlentities(get_lang('ConfirmYourChoice'))).'\');"><img src="'.$this->browser->get_path(WEB_IMG_PATH).$img.'"  alt=""/></a>';
		return $link;
	}
	// Inherited
