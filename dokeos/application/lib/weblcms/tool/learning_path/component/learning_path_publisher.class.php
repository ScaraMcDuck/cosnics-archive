...
class LearningPathToolPublisherComponent extends LearningPathToolComponent
{
	function run() 
	{
		...
		if(!isset($object))
		{	
			$html[] = '<p><a href="' . $this->get_url(array(LearningPathTool :: PARAM_ACTION => LearningPathTool :: ACTION_VIEW_LEARNING_PATHS), true) . '"><img src="'.Theme :: get_common_image_path().'action_browser.png" alt="'.Translation :: get('BrowserTitle').'" style="vertical-align:middle;"/> '.Translation :: get('BrowserTitle').'</a></p>';
			$html[] =  $pub->as_html();
		}
		else
		{
