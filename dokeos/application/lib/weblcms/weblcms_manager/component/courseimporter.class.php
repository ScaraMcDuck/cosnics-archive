...
class WeblcmsCourseImporterComponent extends WeblcmsComponent
{
	...
	function run()
	{
		...
		$this_section='platform_admin';
			...
			$breadcrumbs = array();
			$breadcrumbs[] = array ('url' => $this->get_url(), 'name' => Translation :: get_lang('CourseCreateCsv'));
		...
		if($form->validate())
		{
			...
			$this->redirect(null, Translation :: get_lang($success ? 'CourseCreatedCsv' : 'CourseNotCreatedCsv'). '<br />' .$form->get_failed_csv(), ($success ? false : true));
		}
		else
		{
			$this->display_header($breadcrumbs);
