...
require_once dirname(__FILE__).'/default_course_user_category_table_column_model.class.php';
require_once dirname(__FILE__).'/default_course_user_category_table_cell_renderer.class.php';

class CourseUserCategoryTable
{
	/**
	 * Default table name
	 */
	const DEFAULT_NAME = 'courseusercategory';
	/**
	...
	/**
	 * Constructor. Creates a course user category table.
	 * @param CourseUserCategoryTableDataProvider $data_provider The data provider,
	 *                                                       which supplies the
	 *                                                       learning objects
	 *                                                       to display.
	 * @param string $table_name The name for the HTML table element.
	 * @param CourseUserCategoryTableColumnModel $column_model The column model of
	 *                                                     the table. Omit to
	 *                                                     use the default
	 *                                                     model.
	 * @param CourseUserCategoryTableCellRenderer $cell_renderer The cell renderer
	 *                                                       for the table.
	 *                                                       Omit to use the
	 *                                                       default renderer.
	 */
	function CourseUserCategoryTable($data_provider, $table_name = null, $column_model = null, $cell_renderer = null)
	{
		$this->set_data_provider($data_provider);
		$this->set_name(isset($table_name) ? $table_name : self :: DEFAULT_NAME);
		$this->set_column_model(isset ($column_model) ? $column_model : new DefaultCourseUserCategoryTableColumnModel());
		$this->set_cell_renderer(isset ($cell_renderer) ? $cell_renderer : new DefaultCourseUserCategoryTableCellRenderer());
	...
	function as_html()
	{
		$table = new SortableTable($this->get_name(), array ($this, 'get_course_user_category_count'), array ($this, 'get_course_user_categories'), $this->get_column_model()->get_default_order_column() + ($this->has_form_actions() ? 1 : 0), $this->get_default_row_count(), $this->get_column_model()->get_default_order_direction());
	 ...
	 */
	function get_course_user_categories($offset, $count, $order_column, $order_direction)
	{
		$courseusercategories = $this->get_data_provider()->get_course_user_categories($offset, $count, $this->get_column_model()->get_column($order_column - ($this->has_form_actions() ? 1 : 0))->get_course_user_category_property(), $order_direction);
		$table_data = array ();
		$column_count = $this->get_column_model()->get_column_count();
		while ($courseusercategory = $courseusercategories->next_result())
		{
			$row = array ();
			if ($this->has_form_actions())
			{
				$row[] = $courseusercategory->get_id();
			}
			for ($i = 0; $i < $column_count; $i ++)
			{
				$row[] = $this->get_cell_renderer()->render_cell($this->get_column_model()->get_column($i), $courseusercategory);
			}
			$table_data[] = $row;
		}
		return $table_data;
	...
	{
		return $this->get_data_provider()->get_course_user_category_count();
	}
}
?>