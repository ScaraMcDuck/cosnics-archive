...
class LearningObjectBrowser extends LearningObjectPublisherComponent
{
	...
	{
		return new EqualityCondition(LearningObject :: PROPERTY_OWNER_ID, $this->get_user_id());
	}
	...
	function get_object_count()
	{
		...
		if (count($types) > 1)
		{
			...
			foreach ($types as $t)
			{
				$c[] = new EqualityCondition(LearningObject :: PROPERTY_TYPE, $t);
			}
			$c = new OrCondition($c);
	...
	function get_objects($from, $number_of_items, $column, $direction)
	{
		...
		{
			$buttons = '<a href="'.$this->get_url(array (LearningObjectPublisher :: PARAM_ACTION => 'publicationcreator', LearningObjectPublisher :: PARAM_LEARNING_OBJECT_ID => $object->get_id()), true).'"><img src="'.api_get_path(WEB_CODE_PATH).'img/publish.gif" alt="'.get_lang('Publish').'"/></a>';
			$buttons .= '<a href="'.$this->get_url(array (LearningObjectPublisher :: PARAM_ACTION => 'publicationcreator', LearningObjectPublisher :: PARAM_LEARNING_OBJECT_ID => $object->get_id(), LearningObjectPublisher :: PARAM_EDIT => 1), true).'"><img src="'.api_get_path(WEB_CODE_PATH).'img/editpublish.gif" alt="'.get_lang('EditAndPublish').'"/></a>';
			$row = array ();
			$row[] = '<img src="'.api_get_path(WEB_CODE_PATH).'img/'.$object->get_type().'.gif" alt="'.$object->get_type().'"/>';
			$row[] = '<a href="'.$this->get_url(array (LearningObjectPublisher :: PARAM_ACTION => 'viewer', LearningObjectPublisher :: PARAM_LEARNING_OBJECT_ID => $object->get_id()), true).'">'.htmlentities($object->get_title()).'</a>';
