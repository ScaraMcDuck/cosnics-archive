...
class LearningObjectViewer extends LearningObjectPublisherComponent
{
	function as_html()
	{
		if ($_GET['object'])
		{
			...
			return LearningObjectDisplay :: factory($object)->get_full_html().'<p><a href="'.$this->get_url(array ('publish_action' => 'publicationcreator', 'object' => $object->get_id())).'"><img src="'.api_get_path(WEB_CODE_PATH).'img/publish.gif" alt="'.get_lang('Publish').'" style="vertical-align: middle"/> '.get_lang('PublishThisObject').'</a></p>';
		}
	}
}
?>