	...
	private function category_tree_as_html($tree)
	{
		...
		foreach ($tree as $node)
		{
			...
			if ($id != 0)
			{
				...
				$options[] = '<a href="'.$this->get_url(array (self :: PARAM_ACTION => self :: ACTION_DELETE, self :: PARAM_ID => $id), true).'" onclick="return confirm(\''.addslashes(htmlentities(Translation :: get('ConfirmYourChoice'))).'\');"><img src="'.$this->parent->get_path(WEB_IMG_PATH).'delete.gif"  alt=""/></a>';
			}
