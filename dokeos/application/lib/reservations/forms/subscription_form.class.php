...
class SubscriptionForm extends FormValidator 
{
    ...
    {	
		...
		for($i = 0; $i < 4; $i++)
		{
			...
			if($subscriptions >= $reservation->get_max_users()) break;
    ...
    {
    	//$db_start = DokeosUtilities :: to_db_date($start_time);
    ...
    function build_timepicker_form()
    {
		if($this->reservation->get_timepicker_max() == 0)
			...
			$this->addElement('html', '<br />' . sprintf(Translation :: get('ChooseTime'),$this->item->get_name(), 
				$this->reservation->get_timepicker_min(), $this->reservation->get_timepicker_max()) . '<br /><ul>');
	...
	function allow_create_subscription()
	{
		...
		if($this->validate())
		{
			...
			if($this->reservation->get_type() == Reservation :: TYPE_TIMEPICKER)
			{
				...
				$start_stamp = DokeosUtilities :: time_from_datepicker($subscription->get_start_time());
				$stop_stamp = DokeosUtilities :: time_from_datepicker($subscription->get_stop_time());
			}
			else
			{
				$start_stamp = DokeosUtilities :: time_from_datepicker($this->reservation->get_start_date());
				$stop_stamp = DokeosUtilities :: time_from_datepicker($this->reservation->get_stop_date());
			}
