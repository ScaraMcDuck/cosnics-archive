...
class Subscription extends DataClass
{
	...
	{
		return parent :: get_default_property_names(array (self :: PROPERTY_USER_ID, self :: PROPERTY_RESERVATION_ID, 
					  self :: PROPERTY_START_TIME, self :: PROPERTY_STOP_TIME, self :: PROPERTY_ACCEPTED,
					  self :: PROPERTY_STATUS, self :: PROPERTY_WEIGHT, self :: PROPERTY_QUOTA_BOX));
	}
	...
	function allow_create($user)
	{
		...
		{
			$stamp_sub_start = DokeosUtilities :: time_from_datepicker($this->get_start_time());
			$stamp_sub_end = DokeosUtilities :: time_from_datepicker($this->get_stop_time());
			
			$stamp_res_start = DokeosUtilities :: time_from_datepicker($reservation->get_start_date());
			$stamp_res_end = DokeosUtilities :: time_from_datepicker($reservation->get_stop_date());
