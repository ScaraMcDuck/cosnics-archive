...
class ReservationsManagerQuotaBoxUpdaterComponent extends ReservationsManagerComponent
{
	...
	function run()
	{
		...
		if($form->validate())
		{
			...
			$this->redirect('url', Translation :: get($success ? 'QuotaBoxUpdated' : 'QuotaBoxNotUpdated'), ($success ? false : true), array(ReservationsManager :: PARAM_ACTION => ReservationsManager :: ACTION_BROWSE_QUOTA_BOXES));
		}
		else
		{
