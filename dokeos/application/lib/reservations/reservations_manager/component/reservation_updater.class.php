...
class ReservationsManagerReservationUpdaterComponent extends ReservationsManagerComponent
{
	...
	function run()
	{
		...
		$trail->add(new Breadcrumb($this->get_url(array(ReservationsManager :: PARAM_RESERVATION_ID => $reservation_id, ReservationsManager :: PARAM_ITEM_ID => $item_id)), Translation :: get('Update reservation')));
		...
		if($status == 1)
		{
			...
			$this->redirect('url', Translation :: get($success ? 'ReservationUpdated' : 'ReservationNotUpdated'), ($success ? false : true), array(ReservationsManager :: PARAM_ACTION => ReservationsManager :: ACTION_ADMIN_BROWSE_RESERVATIONS, ReservationsManager :: PARAM_ITEM_ID => $item_id));
		}
		else
		{
	...
	function parse_status($status)
	{
		switch($status)
		{
			...
			case 4: return Translation :: get('NoEqualDatesWithTimepicker');
		}
	}
}
?>