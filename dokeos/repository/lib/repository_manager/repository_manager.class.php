...
class RepositoryManager extends CoreApplication
{
    ...
    * Constant defining an action of the repository manager.
    ...
    function has_right($learning_object, $user_id, $right)
    {
        ...
        $reflect = new ReflectionClass(Application :: application_to_class(self :: APPLICATION_NAME) . 'Rights');
        ...
        foreach ($list as $key => $value)
        {
            if($value['learning_object'] == $learning_object->get_id() && $value['right'] == $right)
                return true;
        }
        return false;
}//has_right
