...
class RepositoryManagerViewerComponent extends RepositoryManagerComponent
{
	function run()
	{
		...
		if ($id)
		{
			...
			echo '<a href="'.$this->get_learning_object_editing_url($object).'" title="'.get_lang('Edit').'"><img src="'.$this->get_web_code_path().'img/edit.gif" alt="'.get_lang('Edit').'" style="vertical-align: middle;"/>'.get_lang('Edit').'</a>';
			echo '</li>';
			$delete_url = $this->get_learning_object_deletion_url($object);
			if (isset($delete_url))
			{
				echo '<li style="display: inline; margin: 0; padding: 0 1ex 0 0;">';
				echo '<a href="'.$delete_url.'" title="'.get_lang('Delete').'"><img src="'.$this->get_web_code_path().'img/delete.gif" alt="'.get_lang('Delete').'" style="vertical-align: middle;"/>'.get_lang('Delete').'</a>';
				echo '</li>';
			}
			echo '</ul>';
