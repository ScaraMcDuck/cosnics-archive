...
class IeeeLomMapper extends MetadataMapper
{
    ...
    private function merge_general_language()
    {
	    ...
	    foreach ($metadata_array as $content_object_metadata) 
	    {
	        ...
	        if(isset($override_id))
	    	{
	    	     ...
	    	     * Remove the original node corresponding to the same id 
	    	     */
	    	    XMLTool :: delete_element_by_xpath($this->ieeeLom->get_dom(), '/lom/general/language[@'. self :: ORIGINAL_ID_ATTRIBUTE . '=' . $override_id . ']');
	    	}
    ...
    public function get_general_languages()
    {
        ...
        foreach ($language_nodes as $language) 
	    {
	        ...
	        $lang_array[self :: METADATA_ID_ATTRIBUTE] = XMLTool :: get_attribute($language, self :: METADATA_ID_ATTRIBUTE, DataClass :: NO_UID);
	        $lang_array[self :: ORIGINAL_ID_ATTRIBUTE] = XMLTool :: get_attribute($language, self :: ORIGINAL_ID_ATTRIBUTE, DataClass :: NO_UID);
	        $lang_array[self :: OVERRIDE_ID_ATTRIBUTE] = XMLTool :: get_attribute($language, self :: OVERRIDE_ID_ATTRIBUTE, DataClass :: NO_UID);
...
//        debug($date_original_id, '$date_original_id');
    ...
    public function get_lifeCycle_contribution()
    {
        ...
        foreach ($contribution_nodes as $contribution) 
	    {
	        ...
	        $contribution_array['contribution_original_id'] = XMLTool :: get_attribute($role, self :: ORIGINAL_ID_ATTRIBUTE, DataClass :: NO_UID);
	        ...
	        foreach($entities as $entity_index => $entity)
	        {
	            ...
	            $contribution_array['entity'][$entity_index]['entity_original_id']                          = XMLTool :: get_attribute($entity, self :: ORIGINAL_ID_ATTRIBUTE, DataClass :: NO_UID);
	             ...
	             */
	            $contribution_array['entity'][$entity_index]['entity_override_id']                          = XMLTool :: get_attribute($entity, 'name_override_id', DataClass :: NO_UID);
	        }
	        ...
	        if(isset($role))
	        {
    	        ...
    	        $contribution_array['role_original_id']                 = XMLTool :: get_attribute($role, self :: ORIGINAL_ID_ATTRIBUTE, DataClass :: NO_UID);
	        }
	        ...
	        if(isset($date))
	        {
    	        ...
    	        $contribution_array['date_original_id']                 = XMLTool :: get_attribute($date, self :: ORIGINAL_ID_ATTRIBUTE, DataClass :: NO_UID);
	        }
