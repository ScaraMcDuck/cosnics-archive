...
class CalendarEventDifference extends LearningObjectDifference
{
	function get_difference()
	{
		...
		$object_string = htmlentities(Translation :: get_lang('From').' '.format_locale_date($date_format,$object->get_start_date()).' '.Translation :: get_lang('Until').' '.format_locale_date($date_format,$object->get_end_date()));
        $object_string = explode("\n", strip_tags($object_string));
           	
        $version_string = htmlentities(Translation :: get_lang('From').' '.format_locale_date($date_format,$version->get_start_date()).' '.Translation :: get_lang('Until').' '.format_locale_date($date_format,$version->get_end_date()));
