...
class FillInBlanksQuestionQtiExport extends QuestionQtiExport
{
	...
	function get_response_xml($answers)
	{
		...
		{
			$response_xml[] = '<responseDeclaration identifier="'.$answer['answer']->get_id().'" cardinality="single" baseType="string">';
	...
	function get_interaction_xml($answers)
	{
		...
		foreach ($answers as $answer)
		{
			$interaction_xml[] = '<textEntryInteraction responseIdentifier="'.$answer['answer']->get_id().'" expectedLength="20" />';
		}
