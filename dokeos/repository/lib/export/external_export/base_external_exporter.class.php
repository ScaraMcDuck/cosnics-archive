<?php
require_once Path :: get_repository_path() . '/lib/export/dlof/dlof_export.class.php';
...
abstract class BaseExternalExporter 
{
	...
	public static function get_instance($export)
	{
	    ...
	    if(file_exists(Path :: get_repository_path() . '/lib/export/external_export/' . strtolower($export_type) . '/custom/' . strtolower($catalog_name)  . '_external_exporter.class.php'))
	    {
	        ...
	        $class_name = DokeosUtilities :: underscores_to_camelcase($catalog_name) . 'ExternalExporter';
	    }
	    else
	    {
	        ...
	        $class_name = DokeosUtilities :: underscores_to_camelcase($export_type) . 'ExternalExporter';
	    }
...
//	    debug($zippath);
	...
	{
	    return uniqid(str_replace('www', '', $_SERVER['HTTP_HOST']) . ':dokeos:');
	}
