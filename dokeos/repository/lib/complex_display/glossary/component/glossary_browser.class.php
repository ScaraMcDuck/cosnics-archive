...
class GlossaryDisplayGlossaryBrowserComponent extends GlossaryDisplayComponent
{
	private $action_bar;
	...
	function run()
	{
		...
		$publications = WeblcmsDataManager :: get_instance()->retrieve_learning_object_publications(Request :: get('course'), null, null, null, new EqualityCondition('tool','glossary'),false, null, null, 0, -1, null, new EqualityCondition('type','introduction'));
		$this->introduction_text = $publications->next_result();

		$this->action_bar = $this->get_action_bar();

		$browser = new GlossaryBrowser($this->get_parent()->get_parent());
		...
		if(!isset($_GET['pid']))
		{
            ...
            {
                echo $this->get_parent()->get_parent()->display_introduction_text($this->introduction_text);
			}
		}
