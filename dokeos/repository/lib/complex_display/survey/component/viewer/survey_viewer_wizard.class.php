...
class SurveyViewerWizard extends HTML_QuickForm_Controller
{
	...
	{
		$survey = $this->survey;
		$total_questions = RepositoryDataManager :: get_instance()->count_complex_learning_object_items(new EqualityCondition(ComplexLearningObjectItem :: PROPERTY_PARENT, $survey->get_id()));
	...
	function get_questions($page_number)
	{
		...
		$questions = RepositoryDataManager :: get_instance()->retrieve_complex_learning_object_items(new EqualityCondition(ComplexLearningObjectItem :: PROPERTY_PARENT, $survey->get_id()), array(), array(), $start, $stop);
		return $questions;
	}
