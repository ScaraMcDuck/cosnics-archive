...
class WikiDisplayWikiPageCreatorComponent extends WikiDisplayComponent
{
	...
	function run()
	{
        ...
        $trail->add(new BreadCrumb($this->get_url(array(WikiDisplay ::PARAM_DISPLAY_ACTION => WikiDisplay :: ACTION_VIEW_WIKI, Tool :: PARAM_PUBLICATION_ID => $this->wiki_id)), DokeosUtilities::truncate_string($wiki->get_title(),20)));
        $trail->add(new BreadCrumb($this->get_url(array(WikiDisplay ::PARAM_DISPLAY_ACTION => WikiDisplay :: ACTION_CREATE_PAGE, Tool :: PARAM_PUBLICATION_ID => $this->wiki_id)), Translation :: get('CreateWikiPage')));
        ...
        $this->pub->set_parameter('pid', $this->wiki_id);
        ...
        else
        {
            ...
            if($count==1)
            {
                ...
                $cloi->create();
                $this->redirect($message, '', array(WikiDisplay ::PARAM_DISPLAY_ACTION => WikiDisplay :: ACTION_VIEW_WIKI_PAGE, Tool :: PARAM_COMPLEX_ID => $cloi->get_id(), 'pid' => $this->wiki_id));
            }
            else
            {
                $this->get_parent()->get_parent()->display_header($trail, true);
