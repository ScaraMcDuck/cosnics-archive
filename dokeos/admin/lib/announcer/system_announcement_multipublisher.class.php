...
class SystemAnnouncerMultipublisherComponent extends PublisherMultipublisherComponent
{
	function get_publications_form()
	{
		...
		if (count($ids) > 0)
		{
			...
			{
				$html[] = '<li><img src="'.Theme :: get_common_img_path().'treemenu_types/'.$learning_object->get_icon_name().'.png" alt="'.htmlentities(Translation :: get(LearningObject :: type_to_class($learning_object->get_type()).'TypeName')).'"/> '.$learning_object->get_title().'</li>';
			}
