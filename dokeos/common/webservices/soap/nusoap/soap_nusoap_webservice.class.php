...
class SoapNusoapWebservice extends Webservice
{
    ...
    function provide_webservice($functions)
    {
        ...
        foreach ($functions as $name => $objects) //runs through all methods
        {
            ...
            $server->register(get_class($this->webservice_handler) . '.' . $name, array('input' => 'tns:' . get_class($in) . ($objects['array_input'] ? 's' : ''), 'hash' => 'xsd:string'), array('return' => 'tns:' . get_class($out) . ($objects['array_output'] ? 's' : '')), 'http://www.dokeos.com', 'http://www.dokeos.com#' . $name, 'rpc', 'encoded', '', '', 'NusoapWebservice.handle_webservice');
