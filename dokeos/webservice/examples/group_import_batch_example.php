...
{
    global $hash, $client;
	log_message('Creating groups ');
	if($hash == '')
    $hash = login();

    $result = $client->call('WebServicesGroup.create_groups', array('input' => $groups, 'hash' => $hash));
    if($result == 1)
    {
        log_message(print_r('Groups successfully created', true));
    }
    else
