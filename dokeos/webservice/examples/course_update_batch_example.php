...
function update_courses($course)
{
	global $hash, $client;
    log_message('Updating courses');
    ...
    $result = $client->call('WebServicesCourse.update_courses', array('input' => $course, 'hash' => $hash));
    if($result == 1)
    {
        log_message(print_r('Courses successfully updated', true));
    }
    else
