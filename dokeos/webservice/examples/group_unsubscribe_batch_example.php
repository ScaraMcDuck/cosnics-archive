...
function unsubscribe_users($group_rel_user)
{
    global $hash, $client;
    log_message('Unsubscribing users from groups');
	...
	if(empty($hash))
    $hash = login();
	$result = $client->call('WebServicesGroup.unsubscribe_users', array('input' => $group_rel_user, 'hash' => $hash));
    if($result == 1)
    {
        log_message(print_r('Users successfully unsubscribed', true));
    }
    else
