...
function unsubscribe_users($course)
{
    global $hash, $client;
    log_message('Unsubscribing users to courses ');
    ...
    $result = $client->call('WebServicesCourse.unsubscribe_users', array('input' => $course, 'hash' => $hash));
    if($result == 1)
    {
        log_message(print_r('Users successfully unsubscribed from courses', true));
    }
    else
