...
class LicenseInstallWizardPage extends InstallWizardPage
{
	function get_title()
	{
		return Translation :: get_lang('Licence');
	}
	function get_info()
	{
		return Translation :: get_lang('DokeosLicenseInfo');
	}
	function buildForm()
	{
		$this->set_lang($this->controller->exportValue('page_language', 'install_language'));
		$this->_formBuilt = true;
		$this->addElement('textarea', 'license', Translation :: get_lang('Licence'), array ('cols' => 80, 'rows' => 20, 'disabled' => 'disabled', 'style'=>'background-color: white;'));
		$this->addElement('checkbox','license_accept','',Translation :: get_lang('IAccept'));
		$this->addRule('license_accept',Translation :: get_lang('ThisFieldIsRequired'),'required');
