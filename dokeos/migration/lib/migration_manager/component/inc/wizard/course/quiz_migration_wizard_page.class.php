...
class QuizMigrationWizardPage extends MigrationWizardPage
{
	...
	/**
	 * Constructor creates a new QuizMigrationWizardPage
	...
	/**
	 * Execute the page
	 * Starts migration for quiz and quiz questions
	 */
	function perform()
	{
				...
				$exportvalues['migrate_courses'] == 1 && $exportvalues['migrate_users'] == 1)
			{
				...
				{	
					...
					foreach($courses as $i => $course)
					{
						...
						$this->migrate('_Quiz', array('old_mgdm' => $this->old_mgdm, 'del_files' => $this->include_deleted_files), array('old_mgdm' => $this->old_mgdm), $course,0);
						$this->migrate('_Quiz_Question', array('old_mgdm' => $this->old_mgdm, 'del_files' => $this->include_deleted_files), array('old_mgdm' => $this->old_mgdm), $course,1);
