			...
			case ActionSelectionSubscribeWizardPage :: ACTION_SUBSCRIBE :
				...
				foreach($users as $user)
				{
					...
					if (!isset($existing_groupreluser))
					{
						...
						{
							Events :: trigger_event('subscribe_user', 'class_group', array('target_class_group_id' => $groupreluser->get_classgroup_id(), 'target_user_id' => $groupreluser->get_user_id(), 'action_user_id' => $this->parent->get_user()->get_id()));
						}
					}
					else
					{
				...
				$this->parent->redirect('url', Translation :: get($message), ($failures ? true : false), array(ClassGroupManager :: PARAM_ACTION => ClassGroupManager :: ACTION_VIEW_CLASSGROUP, ClassGroupManager :: PARAM_CLASSGROUP_ID => $group_id));
				exit;
				break;
		}
